...
def migrate_db():
    ...
    with get_db() as conn:
        ...
        # Удаляем попытку добавить TG_NUMBER, так как он не используется
...
def add_number(number, user_id, tg_group="1"):
    ...
    with get_db() as conn:
        ...
        if result and result[0] == 1:
            print(f"[DEBUG] Пользователь {user_id} в АФК, добавление номера {number} отклонено")
...
def reset_numbers_on_afk(user_id):
    """Сбрасывает номера пользователя в очередь при включении АФК."""
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute('''
            UPDATE numbers
            SET STATUS = 'queued',
                ...
                TAKE_DATE = '0'
            WHERE ID_OWNER = ? 
            AND STATUS IN ('ожидает', 'in_use')
            AND TAKE_DATE != '0'
        ''', (user_id,))
        affected_rows = cursor.rowcount
        conn.commit()
        print(f"[DEBUG] Сброшено {affected_rows} номеров пользователя {user_id} в очередь из-за АФК")
...
def get_afk_status(user_id):
    ...
    with get_db() as conn:
        ...
        cursor.execute('SELECT IS_AFK FROM users WHERE ID = ?', (user_id,))
        result = cursor.fetchone()
        status = bool(result[0]) if result else False
        print(f"[DEBUG] get_afk_status: user_id={user_id}, status={status}")
        return status
...
def toggle_afk_status(user_id):
    ...
    with get_db() as conn:
        ...
        conn.commit()
        print(f"[DEBUG] toggle_afk_status: user_id={user_id}, new_status={new_status}")
        # Сбрасываем номера в очередь при включении АФК
        if new_status:
            reset_numbers_on_afk(user_id)  # Исправленный вызов
        return bool(new_status)
